...
export const GenericFlowDiagram = ({ data }: { data: any }) => {
  return (
    <div className="w-full py-8 flex flex-col items-center gap-4">
      {(data?.steps || ['Input', 'Process', 'Output']).map((step: string, i: number) => (
        <React.Fragment key={i}>
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="px-6 py-3 bg-white border border-stone-200 rounded-lg shadow-sm text-stone-800 font-medium text-sm text-center min-w-[120px]"
          >
            {step}
          </motion.div>
          {i < (data?.steps?.length || 3) - 1 && (
            <motion.div
              initial={{ height: 0 }}
              whileInView={{ height: 20 }}
              viewport={{ once: true }}
              className="w-px bg-nobel-gold/40"
            />
          )}
        </React.Fragment>
      ))}
